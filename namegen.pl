...
if ($#ARGV != 0 ) {
    print "usage: namegen [-d] <# of names>\n";
    exit 1;
}
$num_names = $ARGV[0];

if (!(-f $LAST_NAME_FILE && -f $MALE_FIRST_NAME_FILE && -f $FEMALE_FIRST_NAME_FILE) && !$d) {
    print "Use -d option to download name files from the census.\n";
    exit 1;
}

if ($d) {
    download($LAST_NAME_URL, $LAST_NAME_FILE, "last name data");
    download($FIRST_NAME_FEMALE_URL, $FEMALE_FIRST_NAME_FILE, "female first name data");
    download($FIRST_NAME_MALE_URL, $MALE_FIRST_NAME_FILE, "male first name data");
}
